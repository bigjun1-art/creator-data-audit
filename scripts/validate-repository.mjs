...
const skillName = "creator-data-audit";

function walk(directory) {
  return fs.readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    const full = path.join(directory, entry.name);
    if ([".git", "__pycache__", ".venv", "node_modules"].includes(entry.name)) return [];
...
for (const required of ["run_creator_audit.py", "prepare_run.py", "xingtu_chrome_bridge.mjs", "self_test.py"]) {
  assert(fs.existsSync(path.join(root, "scripts", required)), `scripts/${required} missing`);
}

const textExtensions = new Set([".md", ".py", ".mjs", ".js", ".json", ".yaml", ".yml", ".sh", ".txt"]);
const forbidden = [
...
for (const file of walk(root)) {
  ...
  assert(!/\.bak$|\.har$|\.env$|\.pyc$/.test(file), `${relative}: forbidden artifact type`);
